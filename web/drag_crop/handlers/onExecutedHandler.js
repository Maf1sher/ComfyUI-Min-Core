...
export function handleOnExecuted(node, message) {
  ...
  node.image.onload = () => {
    ...
    if (shouldRecomputeSize) {
      ...
      if (newSize && newSize[0] > 0 && newSize[1] > 0) {
        ...
        console.warn(
          `[MinCore_DragCrop] Node ${node.id}: computeSize returned invalid size:`,
          newSize
        );
    ...
    if (boxPatch) {
      ...
      console.warn(
        `[MinCore_DragCrop] Node ${node.id}: Failed to update drag box from crop values`
      );
    }
  ...
  node.image.onerror = () => {
    ...
    console.warn("[MinCore_DragCrop] Image failed to load");
  };
