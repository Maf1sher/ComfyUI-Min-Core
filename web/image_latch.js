...
const NODE_TYPE = "MinCore_ImageLatch";
...
// node with a new seed) invalidates the cache key of this node, forcing it
// to re-execute — defeating the entire latch mechanism.
//
...
async function _refreshBackupStatus(nodeIds) {
    ...
    try {
        const res = await api.fetchApi(
            `/mincore/image_latch/backup_status?node_ids=${nodeIds.join(",")}`,
            { cache: "no-store" }
        );
        if (res.ok) {
...
async function _queueNode(node) {
    try {
        ...
        console.error("Image Latch queue error:", err);
...
function setupImageLatchNode(node) {
    ...
    async function actionRefresh() {
        ...
        try {
            await api.fetchApi(
                `/mincore/image_latch/refresh?node_id=${encodeURIComponent(String(node.id))}`,
                { cache: "no-store" }
            );
        } catch (_) {}
    ...
    if (node.id != null && node.id >= 0) {
        api.fetchApi(`/mincore/image_latch/status?node_id=${encodeURIComponent(String(node.id))}`, { cache: "no-store" })
            .then(res => res.json())
            .then(data => {
                if (data.latched) {
...
// we remove the `image` link from any ImageLatch node that has block=True
// and a backup file on disk.  This prevents the cache system from including
// upstream ancestors in the cache key.

const _origGraphToPrompt = app.graphToPrompt.bind(app);
app.graphToPrompt = async function () {
    // Collect ImageLatch node IDs that need a status check
    const latchNodeIds = [];
    if (app.graph) {
    ...
    for (const [nodeId, nodeData] of Object.entries(result.output)) {
        ...
        if (block && hasBackup && Array.isArray(inputs.image)) {
            delete inputs.image;
        }
    }
...
app.registerExtension({
    name: "MinCore.ImageLatch",

    async nodeCreated(node) {
        if (node.comfyClass !== NODE_TYPE) return;
        setupImageLatchNode(node);
    },
});
